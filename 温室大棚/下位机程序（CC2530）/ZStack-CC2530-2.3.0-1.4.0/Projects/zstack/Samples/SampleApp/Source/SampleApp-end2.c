        ...
        case ZDO_STATE_CHANGE:
          ...
          if ((SampleApp_NwkState == DEV_ROUTER)|| (SampleApp_NwkState == DEV_END_DEVICE) )
          {
              ...
              HalLedBlink (HAL_LED_1,4, 50, 500);
               P2SEL &= ~0x03;  //P2_0 , P2_1设置为通用io；   
               P2DIR &= ~0x01;  //P2_0设置为输入模式；
               P2DIR |=  0x02;  //P2_1设置为输出模式；
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  {    
    case SAMPLEAPP_PERIODIC_CLUSTERID: //表示周期函数
          ...
          if(coor_to_end[0]==0x02&&coor_to_end[1]==0x03)
          {
              end_sensor[0]=0x02;//地址
              end_sensor[1]=0x03;//功能代码
              temp=get6();//获取光照值
              end_sensor[2]=(char)(temp)%10;
              end_sensor[3]=(char)(temp*10)%10;
              end_sensor[4]=(char)(temp*100)%10;
              end_sensor[5]=(char)(temp*1000)%10;
              gz_ce1=end_sensor[2]*1000+ end_sensor[3]*100+ end_sensor[4]*10+ end_sensor[5];//光照值
        ...
        case  SAMPLEAPP_FLASH_CLUSTERID: //表示参数下发
           osal_memcpy(coor_to_con,pkt->cmd.Data,pkt->cmd.DataLength);   // 对无线数据进行处理
            if(coor_to_con[0]==0x02)
           {
             if(coor_to_con[1]==0x04)
             {
               set_gz=coor_to_con[2]*256+coor_to_con[3];
               end_con[0]=0x02;
               end_con[1]=0x04;
               end_con[2]=0x68;
